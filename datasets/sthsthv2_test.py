...
for train_sample in train_list:
    ...
    if action_label in top25_labels:
        ...
        num_frames = len(glob.glob(os.path.join(frames_dir, video_id, '*.jpg')))
        if num_frames > frame_thres:
            if train_10000_stats.get(action_label, 0) < 400:
                train_10000_samples.append(train_sample)
                train_10000_stats[action_label] = train_10000_stats.get(action_label, 0) + 1

for action_label in train_10000_stats.keys():
    print(action_label, train_10000_stats[action_label])

train_10000_path = os.path.join(proj_root, 'my_sthsthv2_annot', 'train_10000.json')
with open(train_10000_path, 'w') as f:
    json.dump(train_10000_samples, f)
