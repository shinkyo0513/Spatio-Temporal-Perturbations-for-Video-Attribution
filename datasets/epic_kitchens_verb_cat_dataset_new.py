...
class EPIC_Kitchens_Dataset (Dataset):
    def __init__ (self, root_dir, frames_per_clip, sample_mode, 
        ...
        self.rgb_file = join(root_dir, "frames_rgb_flow/rgb/cat_verb_segs")
        if train:
    ...
    def set_seg_bbox_dict (self):
        ...
        for seg_name in self.sltd_list:
            ...
            for fidx_wbbox, bbox in seg_bbox.items():
                y, x, dy, dx = bbox
                x0 = min(math.floor(112*x / 1920), 111)
                y0 = min(math.floor(112*y / 1080), 111)
                x1 = min(math.ceil(112*(x+dx) / 1920), 111)
                y1 = min(math.ceil(112*(y+dy) / 1080), 111)
