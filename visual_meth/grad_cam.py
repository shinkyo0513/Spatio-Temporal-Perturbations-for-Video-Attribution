...
def grad_cam_2d (inputs, labels, model, device, layer_name, norm_vis=True):
    ...
    observ_grad = observ_grad_[0]   # N*numf x C x 56 x 56
    # print('observ_grad:', observ_grad.shape)
    # observ_grad = torch.repeat_interleave(observ_grad, int(nt/observ_grad.shape[2]), dim=2)

    observ_grad_w = observ_grad.mean(dim=3, keepdim=True).mean(dim=2, keepdim=True) # N*numf x C x 1x1
    out_masks = F.relu( (observ_grad_w*observ_actv).sum(dim=1, keepdim=True) ) # N*numf x 1 x 14x14
...
def grad_cam (inputs, labels, model, model_name, device, layer_name, norm_vis=True):
    if model_name == 'r2p1d':
        ...
        # layer_name = ['module', 'model'] + layer_name
        return grad_cam_2d(inputs, labels, model, device, layer_name, norm_vis)
    else:
