...
parser.add_argument("--vis_method", type=str,
                    choices=["g", "ig", "sg", "sg2", "xrai", "la", "eb", "grad_cam", "perturb", "random"])
...
parser.add_argument("--perturb_mode", type=str,
                    default='keep', choices=["remove", "keep"])
...
if not args.only_test:
    ...
    if args.model == "r50l":
        ...
        optimizer_ft = torch.optim.SGD(
            params_to_update, lr=0.001, momentum=0.9)
    ...
    for x in ['train', 'val']:
        ...
        video_datasets[x] = UCF101_24_Dataset(ds_path, args.num_f, sample_mode, num_clips,
                                              ...
                                              noised=args.noised)
    print({x: 'Num of clips:{}'.format(
        len(video_datasets[x])) for x in ['train', 'val']})
    ...
    if model == 'r50l':
        scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer_ft, 15, gamma=0.1)
    else:
        scheduler = None

    # Train
    hist = model_ft.train_model(
        dataloaders, criterion, optimizer_ft, pt_save_dir, args.num_epochs, scheduler)
...
cm_fig.savefig(os.path.join(
    proj_root, 'visual_res', 'cm', f'{save_label}.png'))
