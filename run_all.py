...
def load_model_and_dataset (args):
    if args.dataset == "ucf101":
        ...
        if args.model == "r2p1d":
            ...
            if args.retrain_type == "half":
                model_wgts_dir = f"{proj_root}/model_param/ucf101_24_r2plus1d_18_16_LongRange.pt"
            elif args.retrain_type == "full":
            ...
            if args.retrain_type == "half":
                model_wgts_dir = f"{proj_root}/model_param/ucf101_24_vgg16lstm_16_LongRange.pt"
            elif args.retrain_type == "full":
        ...
        if args.model == "r2p1d":
            ...
            from model_def.r2plus1d import r2plus1d as model
            if args.retrain_type == "half":
                model_wgts_dir = f"{proj_root}/model_param/epic_r2plus1d_18_16_LongRange.pt"
            elif args.retrain_type == "full":
                model_wgts_dir = f"{proj_root}/model_param/epic_r2plus1d_18_16_Full_LongRange.pt"
        elif args.model == "v16l":
            from datasets.epic_kitchens_dataset_vgg16lstm import EPIC_Kitchens_Dataset as dataset
            from model_def.vgg16lstm import vgg16lstm as model
            if args.retrain_type == "half":
                model_wgts_dir = f"{proj_root}/model_param/epic_vgg16lstm_16_LongRange.pt"
            elif args.retrain_type == "full":
    ...
    video_datasets = {x: dataset(ds_path, num_frame, sample_mode, 1, 6, \
                            x=='train', testlist_idx=1) for x in phase_set}
...
def main_worker(gpu, args):
    if args.vis_method == 'perturb':
        ...
        batch_size = 8 if args.model == 'r2p1d' else 2
    ...
    samplers = {x: torch.utils.data.distributed.DistributedSampler(video_datasets[x], 
                        num_replicas=args.world_size, rank=rank) for x in phase_set}
    dataloaders = {x: DataLoader(video_datasets[x], batch_size=batch_size, shuffle=False, 
                        num_workers=0, sampler=samplers[x], pin_memory=False) for x in phase_set}
    print(rank, {x: 'Num of batches:{}'.format(len(dataloaders[x])) for x in phase_set})
        ...
        for samples in dataloaders[phase]:
            ...
            if args.vis_method == 'ig':
                ...
                res = video_perturbation(
                        ...
                        gpu_id=gpu, print_iter=200, perturb_type="fade")[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, choices=['r2p1d', 'v16l'])
    parser.add_argument("--vis_method", type=str, choices=['ig', 'sg', 'sg2', 'sg_var', 'grad_cam', 'perturb'])
    ...
    parser.add_argument("--perturb_niter", type=int, default=1600)
    ...
    if args.vis_method == 'perturb':
        save_label = save_label + f"_{args.perturb_tsigma}"
    if args.extra_label != "":
    ...
    if args.vis_method == 'perturb':
        if args.model == 'r2plus1d':
            args.areas = [0.02, 0.05, 0.1, 0.2]
        elif args.model == 'vgg16lstm':
            args.areas = [0.05, 0.1, 0.2, 0.3]
